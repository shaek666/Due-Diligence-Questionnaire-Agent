...
export EMBEDDINGS_BACKEND=sentence_transformers
export LLM_BACKEND=transformers
export LLM_MODEL_NAME=${LLM_MODEL_NAME:-TinyLlama/TinyLlama-1.1B-Chat-v1.0}

mkdir -p "$DATA_DIR/storage"

PYTHONPATH="$BACKEND_DIR" conda run -n "$ENV_NAME" uvicorn app:app --host 0.0.0.0 --port 8000 --app-dir "$BACKEND_DIR" &
API_PID=$!

PYTHONPATH="$BACKEND_DIR" conda run -n "$ENV_NAME" celery -A src.workers.celery_app.celery_app worker --loglevel=info &
WORKER_PID=$!
